...
The check script writes the indexes into `spec/index/` as Markdown tables: `by-kind.md` lists entries by kind, `by-area.md` by area, `by-status.md` by status, and `references.md` gives, for each entry, the entries that cite or relate to it. Nobody edits them by hand. They are committed so that they can be read on GitHub, and a change that leaves them out of date fails the check.

`spec/glossary.md` has one `##` heading per term, spelled the way the pseudocode spells it, followed by what the term means and, where the game shows the concept to the player, the name the game uses for it. The rest of the entry depends on what the term names:
...
| A value from outside the game | Its type, what the original reads it from, and when it can change (see [Formulas and procedures](#formulas-and-procedures)). |
...
- `resource(...)` refers to a resource, taking the parts of a [resource reference](#notation) as arguments in the order the notation writes them, with a file name in double quotes: `resource("SOUNDS.RES", 12 + n)`, `resource("Chaos Overlords.exe", "BITMAP", 132)`. It is how a rule passes a sound or an image to an event.
- `read_file(path, FMT-SAVE-001)` reads a file and gives a structure of that format, decoded as its layout table describes, and `write_file(path, x)` writes the structure `x` to a file in the layout of its format. `path` is a `char[]` value or a file name in double quotes, written the way the build entry writes paths. A third argument, an offset, reads or writes one structure at that place in the file, for an original that reads a record at a time. Where the file can be missing, shorter than the format or impossible to write, the rule says what the original does then, or lists it as an open question until that is known.
...
recording: null           # a demo or replay the original recorded (see below), or null
repetitions: 200
...
2. Enumerations and flags: a table for each, under a `###` heading that gives the names of the fields it applies to, with the columns `Value | Name | Meaning | Status | Evidence`. Names follow the form described in [Formulas and procedures](#formulas-and-procedures), and the `.ksy` definition uses each one in lower case.
...
- every static finding has at least one location for each build it lists, and every location names a build the finding lists;
...
- every rule a format's tables name is in the format's `related` field;
...
- every glossary entry gives what the table under [Where it lives](#where-it-lives) asks of its kind of term, and every address, order and outside source it claims is followed by evidence IDs that resolve, or by `(unknown)`;
- every rule cites in its `evidence` the evidence of each glossary claim its procedure relies on, and lists in its Open questions section each such claim that is `(unknown)`;
- every name in a procedure that has the form of a neutral name, every name it assigns without `let` that is not a local, every value from outside the game it reads, every function, table, clock and generator it uses other than the built-in ones, every queue it drains, and every event it emits has a glossary entry;
...
- every neutral name a procedure uses is the one from its first build and is listed in its Open questions section, and no procedure contains an address outside a neutral name;
...
- every resource a screen or procedure references, and every file a procedure reads or writes, is in a file that a format entry lists;
- every binary format entry's `definition` exists unless the status is `unknown`, and every text format entry's `definition` and `size` are null;
...
- every `recording` that gives a path outside `recordings/` names a file in the experiment's build;
...
Deviation IDs are never reused or renumbered, the same as spec IDs. A deviation that is dropped keeps its heading, and its Dropped item gives the date, written `YYYY-MM-DD`, and the reason in place of `no`, so a comment or row that cited it can still find out what happened.
...
The spec check script also checks `PARITY.md`: every eligible spec entry has exactly one row, the copied columns match the spec, every status follows the table above, every listed test file exists and mentions the row's ID, no `complete` row has its ID in a `PLACEHOLDER:` comment, every listed deviation exists in `DEVIATIONS.md` and names the row in its Departs from, every deviation is listed in each row it names, and the counts at the top are right.

It checks the rest of the implementation's references too. Every spec ID and deviation ID in the code, the tests, `PARITY.md` and `DEVIATIONS.md` must resolve, and a citation of a superseded spec entry fails the check until it is moved to what replaced it, so code does not keep pointing at a claim that was withdrawn. Deviation IDs are unique and follow the ID form, every deviation has its list items in order and departs from at least one entry that has a row, its Default follows the rules for deviations above, and no deviation heading that exists on the main branch is removed.

Most listed tests need the original game's files, which cannot be committed: the art for a screen, the per-unit statistics a combat fixture depends on, the files a decoder reads. Tests find them under the directory named by the `GAME_DIR` environment variable. It holds one directory per build, named by the build's ID, with the files laid out as the build entry's paths give them and a file from a disc under a directory named after the disc (`CD`, `CD2`): `GAME_DIR/BLD-GOG-EN-1.1/Chaos Overlords.exe`. Next to them, `GAME_DIR/captures/` holds the dumps, captures and recordings that cannot be committed, each named by its SHA-256. A test checks every file it reads against the hash its build entry or its finding gives. A test that needs a build or a capture that is not there reports itself as skipped. A format test needs every build its entry lists, and a test that replays a fixture needs the experiment's build, so the copy on the main branch's runner holds every build that a `validated` row's tests use.
