...
`spec/glossary.md` has one `##` heading per term, spelled the way the pseudocode spells it, followed by what the term means and, where the game shows the concept to the player, the name the game uses for it. A term for a structure gives the ID of its format entry, and a term for a list gives the format ID of its elements and the order it keeps them in. Where that format entry has been split by build, the term gives every entry of the split with the builds each one covers. A term for any other value gives its type and where the game keeps it: a field of a structure, by format ID and field name, or a global, by its address in each build that has it. A function, table or clock defined in a rule is a term as well, and its entry gives the ID of that rule. So is each random number generator, whose entry gives the ID of the rule that specifies it and the term for its state. So is every event or message a procedure emits. Its entry lists the arguments it carries, in order, and the rules whose When it runs section names it, in the order they run, and says whether they run at once or names the queue the event waits on. A queue is a term too, and its entry gives the rule that drains it (see [Formulas and procedures](#formulas-and-procedures)).
...
- every status is from the list for its kind, every entry cites what its status requires, counting any source, and a finding or experiment only when it lists the first build in the entry's `builds`, every `reproduced` entry names someone in `reproduced_by` other than its `recorded_by`, and every other finding and experiment has an empty `reproduced_by`;
- every build a `supported` or `established` entry lists is covered by the evidence it cites, every static finding has at least one location for each build it lists, and every experiment lists exactly one build;
- every packed file in a build entry gives its packer and the size, hash, format and unpacking tool of its unpacked form, and every pattern in a format entry's `files` matches a file in each build the entry lists;
- no entry that is not superseded cites a superseded entry, every entry that is not superseded has an empty `superseded_by`, every superseded entry names what replaced or disproved it, a finding's or experiment's `superseded_by` names only findings and experiments, a source appears in `superseded_by` only in a bug's, and no chain of `superseded_by` links leads back to where it started;
- `conflicting` holds only findings and experiments and is empty unless the status is `disputed`, `related` links only to the kinds allowed for the entry, every rule a procedure calls or takes a function or table from, or names in a `# may run:` comment, is in its `related` field (all the entries of a split rule), and so is every format whose structures it reads, writes or makes with `new`, and every rule a format's tables name is in the format's `related` field;
...
- every glossary term that holds a value other than a structure or list gives its type and where the game keeps it, every event's glossary entry lists its handlers and says whether they run at once or names the queue it waits on, every queue's entry names the rule that drains it, and every generator's entry names the rule that specifies it;
- every name in a procedure that has the form of a neutral name, every name it assigns without `let` that is not a local, every function, table, clock and generator it uses other than the built-in ones, every queue it drains, and every event it emits has a glossary entry, every neutral name it uses is the one from its first build, no list of fixed length is given to `append`, `insert` or `remove_at`, every field name whose structure has a known type is in that format entry's layout, no procedure contains an address outside a neutral name, and every neutral name a procedure uses is listed in its Open questions section;
...
Most listed tests need the original game's files, which cannot be committed: the art for a screen, the per-unit statistics a combat fixture depends on, the files a decoder reads. Tests find them in a directory named by the `GAME_DIR` environment variable, check them against the hashes in one of the build entries the row's spec entry lists, and report themselves as skipped when there is no copy.
