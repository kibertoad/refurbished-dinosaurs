...
spec/
  ...
  builds/          BLD-*.md   the exact original files the spec applies to, each with a BLD-*.files.yaml manifest
...
The check script writes the indexes into `spec/index/` as Markdown tables: `by-kind.md` lists entries by kind, `by-area.md` by area, `by-status.md` by status, and `references.md` gives, for each entry, the entries that cite or relate to it. Nobody edits them by hand. They are committed so that they can be read on GitHub, and a change that leaves them out of date fails the check. An index that would pass the [size limit](#file-size) is split into a directory the way that section describes: `spec/index/references/COMBAT/RULE.md`.
...
The spec describes layouts and behaviour, and it holds what a strategy guide or a wiki holds: names, numbers, formulas and tables. What it never keeps is a substantial copy of the game's content, which this page uses to mean three things. The game's writing is the prose it shows or ships, such as dialogue, descriptions, messages, help text and the manual: an entry may quote a short passage as evidence, and refers to anything longer by resource. The game's art is its images, sounds, music, video and maps, which the spec references by resource and never embeds. The game's code is the executable's instructions and the scripts its designers wrote: the spec says what they do in its own words and pseudocode, and never copies a meaningful slice of them, whether as disassembly, decompiler output or a transcription of a script. Names are not content. The names the game gives its concepts and mechanics, such as Influence or Crackdown, and the names of the things a designer made, such as a unit, an item, a site or a character, may be used anywhere in the spec: as terms, in enumeration names, and in the text of entries. Numbers are not content either. Constants the code does arithmetic with are part of the rules and are written down in full. That includes the multipliers and thresholds in a formula, tables in the executable's data that a formula indexes by a number, such as a damage falloff curve indexed by distance or a sine table indexed by angle, and tables keyed by a game entity, such as the per-unit or per-item statistics a designer filled in. A table of more than 64 values goes in a [value file](#file-size). Scripts are code, and get the treatment described above. Where a game runs logic the designers wrote as bytecode in its own interpreter, the interpreter is specified like any other procedure, with rules for what each instruction does, and the script format is a format entry. The scripts themselves are not transcribed into pseudocode. A finding may name a script and say what it does as evidence about the interpreter.

## File size

Every Markdown file this standard defines is at most 1,000 lines long. That covers the entries, the glossary files, `spec/README.md`, the [deviation files](#deviation-log) and the [parity files](#parity-matrix), and the files the check script writes as well. A long file is hard to review in a diff, and when many records share one file, two branches that each add a record to it conflict. Machine-readable files (Kaitai definitions, fixtures, save patches, build manifests and value files) are not counted, since their length follows from the original's files or the runs they record.

Data that has a length of its own, which the entry only describes, is kept in machine-readable files beside the entry, so that it never decides the entry's length. A build lists its files in a [manifest](#builds). Two kinds of table go in a value file: a list of more than 64 values in a procedure (see [Rules, functions, tables and clocks](#rules-functions-tables-and-clocks)), and an enumeration table that would take its format entry past the limit. A value file is a CSV file as [RFC 4180](https://www.rfc-editor.org/rfc/rfc4180) defines it, in UTF-8, in the entry's directory, named after the entry ID and the table, with a header row of column names. An enumeration table's value file has the table's columns and one row per value, `formats/FMT-DATA-009.unit_class.csv`, and the table's `###` heading stays in the entry, followed by a sentence naming the file. A value file holds only what the entry could have held, so the rules on content apply to it unchanged.

Every table the script writes has at most one row per entry, and a row is one line: where a row lists several IDs, they go in one cell, separated by commas. A generated file that would pass the limit becomes a directory of the same name, without `.md`, with one file per area that has rows, named after the area, and one file, `BLD-SRC.md`, for builds and sources, which have no area: `spec/index/references/COMBAT.md`. An area's file that would still pass becomes a directory with one file per kind, `spec/index/references/COMBAT/RULE.md`, and `BLD-SRC.md` becomes `BLD-SRC/BLD.md` and `BLD-SRC/SRC.md`. A kind's file that would still pass becomes a directory of files that each cover a block of 100 numbers, named after the block's first number padded to three digits: `COMBAT/RULE/000.md` for `RULE-COMBAT-000` to `RULE-COMBAT-099`, and `COMBAT/RULE/1000.md` for `RULE-COMBAT-1000` to `RULE-COMBAT-1099`. A builds or sources file that would still pass is split by the first character of the alias instead: `BLD-SRC/SRC/M.md`. A block never has more than 100 entries, so its file stays far under the limit however many entries its area gets. Only blocks with rows are written. Each file in `spec/index/` and `parity/` opens with its path as a `#` heading, relative to `spec/index/` for an index and to `parity/` for a parity file, without `.md`: `# references/COMBAT/RULE/000`, `# COMBAT`. The script splits a file only as far as the limit requires, so a spec always produces the same files, and once a kind is split into blocks, adding entries only adds blocks and never moves an entry from one file to another.

An entry that would pass the limit is split by what it describes, never cut at the line where it reached the limit into a first and second part. The entry keeps its ID, the parts moved out of it become entries with IDs of their own, and each part cites the evidence for what it says:
...
- A value from a format's enumeration table may be written by its name from the Name column, such as `if site.kind == SITE_BAR`, and means the number in its Value column. Enumeration names are upper-case letters, digits and underscores, start with a letter, and are unique across the spec, apart from the entries of a split format, which define the same names. A name describes the value in plain words: a category, as `SITE_BAR` does, or the one thing a designer made that the value stands for, such as one unit type or one named character, by the name the game gives it: `UNIT_ARCHER`. The Value column holds what the original compares, so the name labels the number and never replaces it, and two values the game names alike get names that tell them apart. Where the game gives a value no name, the name is the field's name and the value: `UNIT_TYPE_3`. An enumeration name always leads to one row of one format entry, and a rule that uses one lists that format in its `related` field.
...
- A table that more than one rule uses is defined once, in a rule, with `table falloff: UINT8[4] = [100, 75, 50, 25]` before its procedure, and a finding in the rule's evidence gives the address it was found at, as for any other constant. Other rules use it by name. A table of more than 64 values takes them from a [value file](#file-size) in `rules/` instead of a list: `table sine: INT16[1024] from "RULE-MATH-003.sine.csv"`. The file has the single column `value` and as many rows as the type's count, one per element in index order, and each value is converted to the element type as a list's would be.
- Constants from the executable appear by value, and a finding in the rule's evidence gives the address each was found at. A table the code indexes, used by one rule only, is written as a list assigned to a local at the start of the procedure: `let falloff: UINT8[4] = [100, 75, 50, 25]`. A list of more than 64 values is a `table` definition with a value file instead, even when one rule uses it.
...
- A `char[n]` or `char[]` field is a string. The comparison operators compare two strings byte by byte as unsigned values, up to the first NUL or the end of the field, the way C's `strcmp` does. A rule where the original compares in another way, ignoring case or through a collation table, writes that comparison out as a function. A string literal is written in double quotes. A procedure uses one where the original compares or builds a string itself, such as a file name, a key or a name the game matches, and in the arguments given to `resource`, `read_file`, `write_file` and `sprintf`. Text the game shows is referenced with `resource` where the game takes it from one, and a procedure never holds more of the game's writing than a short label.
...
The manifest is a YAML file next to the entry, named after its ID, that lists the build's files. A CD game can ship hundreds of them, so they are kept out of the entry, the way No-Intro and Redump keep the hashes of a dump in a DAT file apart from any description of the game. The build's `files` list is the manifest's only key, and it lists every file the spec uses, with its format, size and hash. A packed executable adds `packer` and `unpacked` (see [Notation](#notation)). `builds/BLD-GOG-EN-1.1.files.yaml`:
...
Many saves hold content copied from the game's data, such as maps, text or scripts, so a save can usually not be committed (see [Licence](#licence)). The usual starting state is a save patch instead: a JSON file in `saves/`, following the standard's fixture schema, that lists writes to make to a base save.
...
2. Enumerations and flags: a table for each, under a `###` heading that gives the names of the fields it applies to, with the columns `Value | Name | Meaning | Status | Evidence`. Names follow the form described in [Names, structures and pointers](#names-structures-and-pointers), and the `.ksy` definition uses each one in lower case. A table kept in a [value file](#file-size) has the same columns there, and its rows count as the entry's rows everywhere this page speaks of a format's tables.
...
Every listed test is a test of the rebuild; runs of the original itself are experiments, which produce the fixtures. A listed test runs without the original game's files wherever its evidence allows, which is when everything it compares with is committed: a fixture of an emulated call, for example, holds the arguments and the end state and nothing else. Other listed tests need content that cannot be committed: a format test reads the shipped files themselves, a screen test compares frames with captures that show the game's images, and a fixture that starts from a base save needs that save from the player's copy. Such a test file carries the comment `needs: GAME_DIR`, in whatever comment syntax its language uses, so the check can tell it apart, and it finds the files under the directory named by the `GAME_DIR` environment variable. It holds one directory per build, named by the build's ID, with the files laid out as the build entry's paths give them and a file from a disc under a directory named after the disc (`CD`, `CD2`): `GAME_DIR/BLD-GOG-EN-1.1/Chaos Overlords.exe`. An audio track is the raw audio the build entry hashes, in that directory under its track name with a `.cdda` extension: `GAME_DIR/BLD-GOG-EN-1.1/CD/track02.cdda`. Next to them, `GAME_DIR/captures/` holds the dumps, captures, recordings and saves that cannot be committed, each named by its hash. A test checks every file it reads against the hash its build entry or its finding gives. A test that needs a build or a capture that is not there reports itself as skipped. A format test needs every build its entry lists, and a marked test that replays a fixture needs the experiment's build, so the copy a maintainer runs them with holds every build that a `validated` row's marked tests use.

The test suite runs on every change and writes a report of which tests passed, failed or were skipped. A failing test fails the build. The original's files can never be uploaded anywhere a CI runner could fetch them, so CI runs without them. There every test without the `needs: GAME_DIR` comment runs and must pass, which is what confirms the unmarked tests of a `validated` row, and the marked tests report themselves as skipped, which does not fail the build. The marked tests run on a maintainer's machine, with a copy the maintainer owns, and the validation record is how that run reaches the repository.

#### Validation record

After a run in which every test in every marked test file of every `validated` row passed and none was skipped, the maintainer records the run with the check script. It writes `VALIDATION.md` at the root of the game repository, next to `PARITY.md`:
...
| Test file | SHA-256 |
|---|---|
| `tests/Formats.Tests/UnitTableTests.cs` | `9b0e4c1d2a3f4e5d6c7b8a9f0e1d2c3b4a5f6e7d8c9b0a1f2e3d4c5b6a7f8e9d` |
```
...
The machine-readable parts (Kaitai definitions, experiment fixtures, save patches, build manifests, value files, schemas and scripts) are code, and are published under the MIT licence, the same as the rebuild, so they can go into another engine's source tree without a second licence to track.
