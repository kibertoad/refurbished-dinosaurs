...
Finding the answers is the one time the original is run before the static work is done, and it takes the run lock like any other run. The emulator harness starts no process of the game, so checking it takes no lock. Each answer names the tool and version that was tried and what happened, and, where the answer is no, what would change it. The record says what is true now, and it is updated whenever a tool, a machine or an emulator changes the answer.
